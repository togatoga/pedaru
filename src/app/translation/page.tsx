...
function TranslationContent() {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      unlisten = await listen<TranslationData>('translation-data', (event) => {
        ...
        win.setTitle(`Translation: ${preview}${event.payload.selectedText.length > 30 ? '...' : ''}`);
      });
  ...
  useEffect(() => {
    if (data?.autoExplain && data.translationResponse && !explanationPoints && !isExplaining && geminiSettings) {
      handleExplain();
    }
  ...
  return (
    <div className="min-h-screen bg-bg-primary text-text-primary flex flex-col">
      ...
      >
        ...
        <span className="text-sm font-medium text-text-primary pointer-events-none">Translation</span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <CollapsibleSection
          ...
          defaultOpen={true}
        >
          {data.translationResponse.points && data.translationResponse.points.length > 0 ? (
