...
export function useTextSelection(
  ...
  const getSelectionInfo = useCallback((): {
    ...
    const spanIndex = parseInt(
      selectedSpan.getAttribute("data-text-index") || "-1",
      10,
    );
    if (spanIndex < 0) {
      return null;
  ...
  const getContextParts = useCallback(
    async (
      ...
      const prevPageText = pageNum > 1 ? await getPageText(pageNum - 1) : "";
      ...
      if (selectionOffset >= 0) {
        ...
        for (const delta of [0, -1, 1, -2, 2, -5, 5, -10, 10, -20, 20]) {
          ...
          if (
            testIndex >= 0 &&
            testIndex + selectedText.length <= fullText.length
          ) {
        ...
        if (selectionIndex === -1) {
          selectionIndex = Math.max(
            0,
            Math.min(estimatedIndex, fullText.length - 1),
          );
        }
      } else {
        ...
        const currentPageRegion = fullText.slice(
          prevPageLength,
          currentPageEnd,
        );
      ...
      let contextAfter = fullText.slice(
  ...
  const triggerTranslation = useCallback(
    async (withExplanation: boolean = false) => {
      ...
      const { contextBefore, contextAfter } = await getContextParts(
        selectedText,
        selectionPage,
        selectionOffset,
      );
